...
const expectedPublicPaths = ["/", "/codes/", "/tier-list/", "/classes/", "/weapons/", "/value-list/", "/about/", "/contact/", "/editorial-policy/"];
const expectedNoindexPaths = ["/privacy/", "/terms/"];
const expectedBlockedPaths = ["/scripts/", "/macros/", "/executor/", "/exploit/", "/guide/", "/updates/"];
...
if (!exists("src/data/game.ts")) {
  ...
  if (!game.includes("verifiedActiveCodes: []")) violations.push("verifiedActiveCodes must remain empty by default");
}
